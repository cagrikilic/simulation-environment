...
namespace gazebo
{
  ...
  class GazeboRosRealsense : public RealSensePlugin
  {
    ...
    private: std::string extractCameraName(const std::string& name);
    private: sensor_msgs::CameraInfo cameraInfo(const sensor_msgs::Image& image, float horizontal_fov);
    ...
    private: std::string depth_camera_name_;
    private: std::string color_camera_name_;
    private: std::string ired1_camera_name_;
    private: std::string ired2_camera_name_;
    private: std::string camera_frame_id_;
  };
}
